...
gulp.task('clone-libphonenumber', ['make-build-dir'], () => gitClone(libphonenumberUrl, 'libphonenumber')
);

gulp.task('clone-closure-compiler', ['make-build-dir'], () => gitClone(closureCompilerUrl, 'closure-compiler')
);

gulp.task('clone-closure-library', ['make-build-dir'], () => gitClone(closureLibraryUrl, 'closure-library')
);

gulp.task('checkout-closure-linter', ['make-build-dir'], () => gitClone(closureLinterUrl, 'closure-linter')
);

gulp.task('checkout-python-gflags', ['make-build-dir'], () => gitClone(pythonGflagsUrl, 'python-gflags')
);
...
gulp.task('build-closure-compiler', ['download-deps'], () => runCommand('ant', ['-f', 'closure-compiler/build.xml']));

gulp.task('build-deps', ['build-closure-compiler']);

gulp.task('build-libphonenumber', () => {
  var args = ['-f', 'build.xml', 'compile-exports'];
  return runCommand('ant', args, {
    cwd: '.'
  });
});
